...
ALL_MEANS = {
    ...
    'vgg16': [131.0912, 103.8827, 91.4953],
...
ALL_STDS = {
    ...
    'mobilenet_v2': [1., 1., 1.],
    'efficientnet_b0': [1., 1., 1.],
    'inception_v3': [1., 1., 1.],
    'swin_transformer': [1., 1., 1.],
    'vgg16': [1., 1., 1.],
    'vision_transformer': [1., 1., 1.],

    'efficientnet_b0_casia': [1., 1., 1.],
}
...
def crop_img_for_finetune(img, target_size):
    B, C, H, W = img.shape
    center_h, center_w = H // 2, W // 2
    
    start_h = max(center_h - target_size // 2, 0)
    end_h = start_h + target_size
    start_w = max(center_w - target_size // 2, 0)
    end_w = start_w + target_size
    
    cropped_tensors = img[:, :, start_h:end_h, start_w:end_w]
