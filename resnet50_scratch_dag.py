...
class Resnet50_scratch_dag(nn.Module):
    ...
    def forward(self, data):
        ...
        conv5_3_1x1_increase = self.conv5_3_1x1_increase(conv5_3_3x3_bnxx) #self.conv5_3_1x1_increase
...
class Resnet50_scratch_dag_E(nn.Module):

    def __init__(self, num_experts):
        super(Resnet50_scratch_dag_E, self).__init__()
        self.meta = {'mean': [131.0912, 103.8827, 91.4953],
                     'std': [1, 1, 1],
                     'imageSize': [224, 224, 3]}
        self.conv1_7x7_s2 = nn.Conv2d(3, 64, kernel_size=[7, 7], stride=(2, 2), padding=(3, 3), bias=False)
    ...
    def forward(self, data):
        ...
        conv5_3_3x3_bn = self.conv5_3_3x3_bn(conv5_3_3x3)
        conv5_3_3x3_bnxx = self.conv5_3_3x3_relu(conv5_3_3x3_bn)
        # conv5_3_1x1_increase = self.conv5_3_1x1_increase(conv5_3_3x3_bnxx) #num_experts
        outs = []
        for ind in range(self.num_experts):
            ...
            conv5_3x = self.conv5_3_relu(conv5_3)
            pool5_7x7_s1 = self.pool5_7x7_s1(conv5_3x)
            classifier_preflatten = self.classifier[ind](pool5_7x7_s1)
            classifier = classifier_preflatten.view(classifier_preflatten.size(0), -1)
            outs.append(classifier)
        classifier = torch.stack(outs, dim=1).mean(dim=1)
        # return classifier, pool5_7x7_s1
        return classifier, outs
...
def resnet50_scratch_dag_E(weights_path=None, num_experts=3, **kwargs):
    """
    load imported model instance
    ...
    Args:
        weights_path (str): If set, loads model weights from the given path
    """
    model = Resnet50_scratch_dag_E(num_experts)
    if weights_path:
        state_dict = torch.load(weights_path)
        model.load_state_dict(state_dict)
    return model