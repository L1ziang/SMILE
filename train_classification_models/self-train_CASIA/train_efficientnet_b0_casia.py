...
def main(args):
    ...
    totalset = torchvision.datasets.ImageFolder("/root/autodl-tmp/CASIA-WebFace", transform=transform)

    trainset_list, testset_list = train_test_split(list(range(len(totalset.samples))), test_size=0.05, random_state=666)
    ...
    optimizer = torch.optim.Adam(efficientnet_b0.parameters(), lr=0.001)

    best_acc = 0.0
    for epoch in range(args.epochs):
        train_loss = train_one_epoch(efficientnet_b0, train_loader, optimizer, device, logger)
        test_acc = test(efficientnet_b0, test_loader, device, logger)
        print(f"Epoch {epoch+1}, Train Loss: {train_loss:.4f}, Test Accuracy: {test_acc:.4f}")
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(efficientnet_b0.state_dict(), './models_CASIA/efficientnet_b0_best_model.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--LOG', default='./LOG_CASIA/logs_efficientnet_b0', type=str)
